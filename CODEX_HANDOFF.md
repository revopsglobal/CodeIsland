...
- **Entitlements:** app is **NOT sandboxed**, but its hardened runtime still requires `com.apple.security.personal-information.calendars` for EventKit Calendar prompts. `tccd` otherwise denies with `promptPolicy = 4` before showing a prompt. Keep that entitlement in `CodeIsland.entitlements`; privacy usage strings still belong in `Info.plist`.
