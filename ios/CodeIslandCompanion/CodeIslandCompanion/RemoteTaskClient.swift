...
final class RemoteTaskClient: ObservableObject {
    ...
    func sync(baseURL: URL, bearerToken: String, force: Bool = false) async -> RemoteTaskSyncResult {
        ...
        do {
            try await flushOutbox(baseURL: baseURL, bearerToken: bearerToken)
            try await refreshSnapshot(baseURL: baseURL, bearerToken: bearerToken)
            retryAttempt = 0
            nextRetryAt = .distantPast
            lastError = nil
            return .success
    ...
    private func flushOutbox(baseURL: URL, bearerToken: String) async throws {
        for initialDraft in store.drafts {
            ...
            for attachment in draft.attachments where !attachment.uploaded {
                ...
                let response = try await transport.data(for: upload)
                try validate(response)
    ...
    private func refreshSnapshot(baseURL: URL, bearerToken: String) async throws {
        let request = try authenticatedRequest(
            baseURL: baseURL,
            path: "/api/tasks",
            method: "GET",
