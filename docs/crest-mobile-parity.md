...
- Latest strict-gate diagnostic source: PR #105 merged as
  ...
  --check`, and `graphify update . --no-viz`.
- Latest strict E2E report at `2026-07-19T05:08:46Z` returned
  ...
  passed. The installed Mac app is now the PR #109 DMG build running as PID
  `49608`. The latest TestFlight gate remains stale: the physical iPhone is
...
| TestFlight distribution | Ready: signed archive/upload pipeline, source plus compiled App Intent metadata validation, internal group, 60-minute Apple indexing window, and always-preserved IPA artifact. Latest ready build is `1.0.0 (20260719042243)` from run `29673216426`, Apple `VALID`, artifact `8438019701`. | Last physically opened build is `1.0.0 (20260718212803)`, confirmed by paired `clientVersion = 1.0.0` plus exact `clientBuild`; stability, background ActivityKit token lifecycle, reviewed answer, and final-host reauthentication passed physically. Latest ready build `20260719042243` is not physically accepted until Buddy opens on the iPhone and checks in. | Open latest build `20260719042243` from TestFlight on the physical iPhone, then rerun `scripts/report-strict-physical-e2e.sh`; clean expanded Dynamic Island artwork and cellular action remain separate runtime gates |
