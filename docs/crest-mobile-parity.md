...
- Latest TestFlight delivery after PR #126: workflow run `29676579062` from
  ...
  `8439115024`.
- Latest away-readiness verifier now fetches the private web root with a
  Mobile Safari user agent and fails closed unless the served fallback includes
  the attention sections plus iPhone/PWA markers: mobile viewport,
  Apple standalone web-app metadata, safe-area layout, 44px touch targets,
  live status feedback, inline review sheet, and no native browser
...
| Private web fallback | Ready and live-exercised from a headless browser client: responsive authenticated Home/Work/Code, exact actions, Calendar CRUD/Join, confirmed System refresh, exact-byte Downloads transfer, retry/offline state, and replay protection. Away-readiness now also requires the served shell to include iPhone/PWA markers, safe-area layout, touch targets, live status feedback, inline review sheets, and no browser dialogs. | Physical-iPhone browser and cellular-only use remain unverified | Toggle iPhone Wi-Fi off and repeat one action plus file read in the physical iPhone browser over cellular Tailscale |
| TestFlight distribution | Ready: signed archive/upload pipeline, source plus compiled App Intent metadata validation, internal group, 60-minute Apple indexing window, and always-preserved IPA artifact. Latest ready build is `1.0.0 (20260719063154)` from run `29676579062`, Apple `VALID`, artifact `8439115024`. | Last physically opened build is `1.0.0 (20260718212803)`, confirmed by paired `clientVersion = 1.0.0` plus exact `clientBuild`; stability, background ActivityKit token lifecycle, reviewed answer, and final-host reauthentication passed physically. Latest ready build `20260719063154` is not physically accepted until Buddy opens on the iPhone and checks in. | Open latest build `20260719063154` from TestFlight on the physical iPhone, then rerun `scripts/report-strict-physical-e2e.sh`; clean expanded Dynamic Island artwork and cellular action remain separate runtime gates |
