...
| Notifications | Partial by deliberate platform boundary: CodeIsland action-required alerts are prioritized, deduped, redacted, and separated; macOS exposes no public cross-app Notification Center history API, so CodeIsland does not read private databases or request Full Disk Access | Physical registration proof on the replacement build: production APNs and ActivityKit push-to-start tokens reached the paired Mac. Opaque pending/resolved routing, token rotation, authenticated detail refresh, privacy-redacted remote start, and remote end are implemented | Prove physical background delivery, visible Live Activity/Dynamic Island, resolved cleanup, and stale-push behavior |
...
| AI Coding sessions/approvals/questions | Physical Mac proof during the unlocked 1.0.41 run: authenticated Sessions rendered four current sessions with no discovery-card substitution; signed 1.0.43 is now installed and healthy with the same pairing, but its post-install visual pass is blocked by the Mac lock | Physical partial on final build `20260718053347`: authenticated heartbeat plus production APNs and ActivityKit push-to-start registration prove install/open and pairing. Three earlier real approval decisions and one audited question answer remain proven; replacement Sessions rendering, background Live Activity, replay, and cellular use still need final physical proof | Unlock the iPhone surface, verify authenticated Sessions and stability, then run replay plus background and cellular/Tailscale action tests away from the Mac |
